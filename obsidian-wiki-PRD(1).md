...
- Register or pair a plugin device.
- Return a scoped credential/token.
...
Do not store raw long-lived device secrets if avoidable. Store secure token hashes or use short-lived access/refresh token mechanisms appropriate to the auth architecture.
